...
;; streacheble width y heignt: falso para que panel no se expanda
(define mapa (new vertical-panel%
                  [parent all]
                  [min-width 800]	 
                  [min-height 500]
                  [stretchable-width #f]	 
...
(define (vec list-c)
  (vector (string->number(car (posi list-c)))
          (string->number (cadr (posi list-c)))))
...
(define (add-peso lista)
       (cond ((equal? (length lista) 1)
         (send mapaa add-marker (vector (string->number (second (caar lista))) (+ 1 (string->number (third (caar lista))))) (number->string (second lista)) 1 (make-color 255 51 153)))
        (else (send mapaa add-marker (vector (string->number (second (caar lista))) (+ 1 (string->number (third (caar lista))))) (number->string (second (cadr lista))) 1 (make-color 255 51 153)))
        ))

(define (trazar-ruta lista)
  (add-peso lista)
  (send mapaa add-track (list (vector (string->number (second (caar lista))) (string->number (third (caar lista))))
                              (vector (string->number (second (caadr lista)))  (string->number (third (caadr lista))))) #f)
